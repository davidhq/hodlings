...
(function(){
  var rest, timespan, ref$, map, filter, lines, sum, each, path, crypto, homedir, chalk, readFileSync, writeFileSync, existsSync, mkdirSync, watchFile, dataUrl, client, Promise, portfolio, cliOptions, options, renderer, dataDir, lastValuesFile, execute, display, lastRows, displayLatestValues, interval;
  rest = require('rest');
  timespan = require('timespan');
  ref$ = require('prelude-ls'), map = ref$.map, filter = ref$.filter, lines = ref$.lines, sum = ref$.sum, each = ref$.each;
  path = require('path');
  crypto = require('crypto');
  homedir = require('homedir');
  chalk = require('chalk');
  ref$ = require('fs'), readFileSync = ref$.readFileSync, writeFileSync = ref$.writeFileSync, existsSync = ref$.existsSync, mkdirSync = ref$.mkdirSync, watchFile = ref$.watchFile;
  dataUrl = "https://api.coinmarketcap.com/v1/";
  client = rest.wrap(require('rest/interceptor/mime')).wrap(require('rest/interceptor/errorCode')).wrap(require('rest/interceptor/retry'), {
  ...
  portfolio = require('./lib/portfolio');
  ...
  lastValuesFile = path.join(dataDir, './data.json');
  execute = function(cb){
