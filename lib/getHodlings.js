...
export default function getHodlings({ portfolio, baseCurrency, snapshotFilePath = path.join(dataDir, 'valueSnapshot.json'), showProgress = false }) {
  ...
  // +2 is because we make additional two requests for btc and eth price
  // although we might get cache hits if portfolio already contained these two (see fetchMakerData.js)
  const progress = showProgress ? new Progress({ coinsNumber: portfolio.length + 2 }) : null;

  // 2. Get current market situation from coingecko

  return new Promise((success, reject) => {
    fetchMarketData({ baseCurrency, hodlingsPortfolioIdentifiers, progress })
      .then(({ globalData, coinsMarketData, ethBtcMarketData, problems }) => {
        ...
        const hodlings = { portfolio, coinsMarketData, ethBtcMarketData, totalsData, globalData };
