...
function renderHodlings({ hodlingsPath, isDefaultHodlingsPath, baseCurrency, focus }) {
  // 1. Read portfolio
  ...
  const showPortfolioPath = isDefaultHodlingsPath ? chalk.green(hodlingsPath) : chalk.magenta(hodlingsPath);
  console.log(`Read portfolio from ${showPortfolioPath} → ${chalk.cyan(portfolio.length)} coins`);
