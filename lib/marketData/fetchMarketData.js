...
function mapHodlingsPortfolioToCoingecko({ hodlingsPortfolioIdentifiers, coingeckoCoinList }) {
  ...
  for (const identifier of hodlingsPortfolioIdentifiers) {
    ...
    // we report missing coin to the user elsewhere
    if (coin) {
        ...
        singleCoinData({ coinId, cacheDir })
          .then(coinData => {
            try {
              ...
              success({ ...{ symbol: symbol.toUpperCase(), name, id, currency: baseCurrency }, ...dataPoints });
            } catch (e) {
              console.log('Error:');
              console.log(e);
              reject(e);
      ...
      .then(_globalData => {
        ...
        coinList({ cacheDir })
          .then(coingeckoCoinList => {
            ...
            const coingeckoIDs = mapHodlingsPortfolioToCoingecko({ hodlingsPortfolioIdentifiers, coingeckoCoinList });

            // unknown coins in our portfolio - move progress bar for those now since fetching won't move it
            for (let i = 0; i < hodlingsPortfolioIdentifiers.length - coingeckoIDs.length; i++) {
              progress.tick();
            }
