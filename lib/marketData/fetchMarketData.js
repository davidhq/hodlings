...
import Gecko from './coingecko/index.js';
          ...
          .then(coingeckoCoinList => {
            ...
            Promise.all(promises)
              .then(coinsMarketData => {
                const validCoins = sortCoins(coinsMarketData).filter(({ price }) => price);
                const invalidCoins = coinsMarketData.filter(({ price }) => !price);

                for (const coin of invalidCoins) {
                  problems.push(
                    colors.red(
                      `⚠️  Excluded portfolio entry ${colors.yellow(coin.hodlingsIdentifier)} because CoinGecko api entry for it (${colors.yellow(
                        coin.id
                      )} / ${colors.yellow(coin.name)}) had missing price - ${colors.white(
