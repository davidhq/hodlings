...
function readPortfolio(hodlingsPath) {
  ...
  // read portfolio from text file and allow multiple listings of the same symbol, add amounts in this case
  // example:
  // ETH: 2 # account 1
  // ETH: 3 # account 2
  return lines.reduce((portfolio, line) => {
    let [symbol, amount] = line.replace(/\s+/, '').split(':');

    amount = parseFloat(amount);

    const match = portfolio.find(entry => entry.symbol == symbol);
